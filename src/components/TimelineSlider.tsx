...
export const TimelineSlider = ({ currentYear, onYearSelect }: TimelineSliderProps) => {
  ...
  const scrollToYear = (year: number) => {
    ...
    const targetElement = Array.from(yearElements).find(
      el => el.getAttribute('data-year') === year.toString()
    );
    if (targetElement) {
  ...
  const getYearData = (year: number) => {
    return vietnamData.find(d => d.year === year);
  };
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="absolute -top-10 left-1/2 -translate-x-1/2 bg-primary text-primary-foreground px-6 py-2 rounded-t-xl font-semibold hover:bg-primary/90 transition-all duration-300 shadow-lg flex items-center gap-2"
      >
      ...
      <div className="max-w-7xl mx-auto px-6 h-full flex flex-col justify-center">
        {/* Year navigation and info */}
        <div className="flex items-center justify-between mb-3">
          <button
            onClick={handlePrevYear}
            disabled={selectedYear === minYear}
            className="p-2 rounded-full bg-secondary/20 hover:bg-secondary/40 disabled:opacity-30 disabled:cursor-not-allowed transition-all duration-300"
          >
          ...
          <div className="flex items-center gap-6">
            <div className="text-center">
              <div className="text-4xl font-bold text-primary animate-pulse">{selectedYear}</div>
              {currentData && (
              ...
              return (
                <button
                  ...
                  className={`relative flex-shrink-0 transition-all duration-300 ${
                    isSelected
                      ? 'w-16 h-12 bg-primary text-primary-foreground scale-110 shadow-lg'
                      : isMilestone
                      ? 'w-12 h-10 bg-secondary/30 hover:bg-secondary/50'
                      : 'w-8 h-8 bg-muted/30 hover:bg-muted/50'
                  } rounded-lg flex flex-col items-center justify-center text-xs font-bold`}
                  title={`${year}\n${data ? `Dân số: ${(data.population / 1000000).toFixed(1)}M\nGDP: $${data.gdpBillion}B` : ''}`}
                >
                  {(isSelected || isMilestone) && (
                    <span className={isSelected ? 'text-sm' : 'text-xs'}>{year}</span>
                  )}
                  
                  {/* Progress bar below each year */}
                  {data && (
        ...
        {isExpanded && currentData && (
          <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4 animate-fade-in">
            <div className="bg-primary/10 p-3 rounded-lg">
              ...
              <div className="text-lg font-bold text-primary">{(currentData.population / 1000000).toFixed(1)}M</div>
            </div>
            <div className="bg-secondary/10 p-3 rounded-lg">
              <div className="text-xs text-muted-foreground mb-1">GDP</div>
              <div className="text-lg font-bold text-secondary">${currentData.gdpBillion}B</div>
            </div>
            <div className="bg-accent/10 p-3 rounded-lg">
              <div className="text-xs text-muted-foreground mb-1">Đô thị hóa</div>
              <div className="text-lg font-bold text-accent">{currentData.urbanPopPercent}%</div>
            </div>
            <div className="bg-primary/10 p-3 rounded-lg">
              <div className="text-xs text-muted-foreground mb-1">Tuổi thọ</div>
              <div className="text-lg font-bold text-primary">{currentData.lifeExpectancy} tuổi</div>
            </div>
          </div>
        )}
      </div>
